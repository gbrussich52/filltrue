"""Entry band + spine exits. Take profit at the *right time*, not never, not a coupon.

A short put's P&L cannot exceed the credit. That is math, not a rule.

We harvest when the reason to hold is gone:

1. Stop-loss when mark ≥ 1.5× credit (wrong — cut it).
2. Trail giveback after a real run (winner turned — take it).
3. Thesis dead while green (`conditions_still_fit=False` and profit > 0).
4. Time / gamma stop at 21 DTE.

We do **not** close solely because profit hit 50% or 90% while the setup
is still valid and time remains. That is a coupon, not a decision.
"""
...
def decide_exit(
    *,
    ...
    """Return should_close / rule.

    Closes when the *reason to hold* is gone. Does not close solely because
    the profit number is large.
    """
    ...
    if conditions_still_fit is False and pf > 0:
        return {
            "should_close": True,
            "reason": (
                f"thesis dead while green: profit {pf:.0%} — "
                f"harvest and wait to re-enter"
            ),
            "peak_profit_frac": peak,
            "profit_frac": pf,
    ...
    return {
        "should_close": False,
        "reason": (
            f"hold (setup still valid): profit {pf:.0%} peak {peak:.0%} "
